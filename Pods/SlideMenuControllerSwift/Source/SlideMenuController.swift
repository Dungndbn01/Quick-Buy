...
open class SlideMenuController: UIViewController, UIGestureRecognizerDelegate {
    ...
    open func initView() {
      ...
      var opacityframe: CGRect = UIScreen.main.bounds
      ...
      if leftViewController != nil {
        var leftFrame: CGRect = UIScreen.main.bounds
    ...
    open override func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator) {
        ...
        mainContainerView.transform = CGAffineTransform(scaleX: 1, y: 1)
    ...
    func setNavigationBar() {
        self.navigationItem.title = "QUICK BUY"
        
        let slideImage = UIImage(named: "Menu_ICO")?.withRenderingMode(.alwaysTemplate)
        var slideButtonItem = UIBarButtonItem()
        
        slideButtonItem = UIBarButtonItem(image: slideImage, style: .plain, target: self, action: #selector(showVC))
        slideButtonItem.tintColor = UIColor.white
        navigationItem.leftBarButtonItem = slideButtonItem
